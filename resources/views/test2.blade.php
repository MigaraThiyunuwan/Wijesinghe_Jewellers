...
// Define a sample set of users with username => password
$users = [
    'admin' => 'password123',
    'user' => 'userpass'
];
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($errors)) {
        // Save user data into a file (users.txt)
        $file = fopen('users.txt', 'a');
        fwrite($file, $username . ',' . password_hash($password, PASSWORD_BCRYPT) . "\n");
        fclose($file);
...
$stmt = $pdo->prepare('SELECT * FROM posts WHERE author = ? ORDER BY created_at DESC');
$stmt->execute([$username]);
