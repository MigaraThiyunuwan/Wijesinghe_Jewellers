    ...
    public function processImage(Request $request)
    {
       ...
       // return $path;
        ...
        if ($outputFilename === null || !File::exists(public_path('storage/' . $outputFilename))) {
            return view('show_image', ['imagePath' => null])->with('error', 'The processed image could not be found or no face was detected.');
            // return redirect()->back()->with('error', 'The processed image could not be found or no face was detected.');
        }
        // Return the processed image path to the view
        return view('show_image', ['imagePath' => '/storage/' . $outputFilename]);
    }
